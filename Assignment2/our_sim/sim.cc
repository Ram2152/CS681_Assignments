...
Sim::Sim(Config config) : receiver(config.num_threads, config.request_buffer_size), worker(config.total_cores, config.thread_buffer_size) {
    ...
    if (config.arrival_time_distribution == TimeDistributionType::UNIFORM) {
        ...
        inter_arrival_time_dist = UniformDistribution(min_arrival_time, max_arrival_time);
    } else if (config.arrival_time_distribution == TimeDistributionType::EXPONENTIAL) {
        float lambda;
        config_file >> lambda;
        inter_arrival_time_dist = ExponentialDistribution(lambda);
    } else {
        float constant_time;
        config_file >> constant_time;
        inter_arrival_time_dist = ConstDistribution(constant_time);
    }
    ...
    if (config.service_time_distribution == TimeDistributionType::UNIFORM) {
        ...
        service_time_dist = UniformDistribution(min_service_time, max_service_time);
    } else if (config.service_time_distribution == TimeDistributionType::EXPONENTIAL) {
        float lambda;
        config_file >> lambda;
        service_time_dist = ExponentialDistribution(lambda);
    } else {
        float constant_time;
        config_file >> constant_time;
        service_time_dist = ConstDistribution(constant_time);
    }
        ...
        switch (current_event->type) {
            case EventType::ARRIVAL:
                ...
                if (receiver.thread_pool.has_idle_thread()) {
                    ...
                    if (receiver.request_queue.size() < receiver.receiver_buffer_size) {
            ...
            case EventType::THREAD_ARRIVAL:
                ...
                if (worker.busy_cores < worker.total_cores) {
                    ...
                    if (worker.thread_queue.size() < worker.thread_buffer_size) {
